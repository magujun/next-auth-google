					...
					<StreetViewPanorama
						...
						onPositionChanged={() => {
							if (ref.current?.context.data.map.streetView.location) {
								var lat = ref.current?.context.data.map.streetView.location.latLng.lat();
								var lng = ref.current?.context.data.map.streetView.location.latLng.lng();

								// console.log(lat, lng, hasStart, streetPoint);
								if (lat && lng && hasStart && streetPoint.lat !== lat && streetPoint.lng !== lng) {
									// setStreetPoint({
									//   lat: lat,
									//   lng: lng,
									// });
